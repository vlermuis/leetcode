...
Runtime: 4 ms, faster than 91.45% of C online submissions for Remove Duplicates from Sorted List II.
Memory Usage: 7.7 MB, less than 100.00% of C online submissions for Remove Duplicates from Sorted List II.
*/
...
struct ListNode* deleteDuplicates(struct ListNode* head){
    ...
    while(head != NULL)
    {
        if (head->next != NULL)
        {
            if ((head->val != head->next->val) && (cur_dup != head->val))
            {
                add_to_list(head->val, &pResult);
            }
            else
            {
                cur_dup = head->val;
...
int data00[] = {1, 2, 3, 3, 4, 4, 5};
int data01[] = {1, 1, 1, 2, 3};
...
int main()
{
    ...
    array_to_list(data00, 7, &pStart0);
